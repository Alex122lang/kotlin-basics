...
enum class MembershipTypes{
    ...
    VIP,
    GUEST;
    // companion object : this references variables that belong
...
// Main class
class NightClub(override val minimumAge: Int, override val vipBenefits: String, val name: String)
    : ClubRules {
    // hold a list of members [{}, {}, {} ...]
    private val currentMembers = mutableListOf<Member>()

    // hold set of banned members {'' , ''}
    ...
    fun addMember(member: Member) {
        ...
        if (canEnter(member)) {
            ...
            // checks on the benefits if member is VIP, REGULAR, GUEST
            ...
            println(
                "${member.name} cannot enter. ${if (member.age < minimumAge) {
                                "Too young"
                            } else {
                                "Banned"
                            }
    ...
    // this function will allow my objects to be entered in the banned section listing
    fun banMember(id: String){
        // adding id to my banned member list
    ...
    fun listCurrentMembers(){
        ...
        currentMembers.forEach {
            println(" ${it.name} ${it.membership}) since ${it.entryTime.format(timeFormatter)}")
        }
    }
}
    ...
    while(true){
        ...
        when(readlnOrNull()?.trim()){
            "1" -> {
                ...
                val age = readString("Enter Member age: ")
                val membership: MembershipTypes? = null
            }
        }
    }
}